set DOCKER_NETWORK=dockerhadoop_default
set ENV_FILE=hadoop.env
set current_branch="3.1.3"
set task=%1
shift  
...
spark_submit() {
	docker build -t spark-submit ./spark-submit \
		&& docker run --network %DOCKER_NETWORK% --env FAST_SUBMIT=0 --env SPARK_SUBMIT_FLAGS="%@" --env-file %ENV_FILE% spark-submit 
}
